...
class Admin extends Model
{
    ...
    public function login($username_or_email = '', $password = '')
    {
        if (!($this->user = $this->validateUser($username_or_email, $password))) {
            ...
            session()->forget(['USER_ID','ROLE_TYPE','IDENTITY','agent_number','sid','LOGIN_HASH']);
            return false;
        }
        // セッション記入
        session(['USER_ID'=>$this->user['USER_ID']]);
        session(['ROLE_TYPE'=>$this->user['ROLE_TYPE']]);
        session(['IDENTITY'=>$this->user['IDENTITY']]);
        session(['agent_number'=>$this->user['agent_number']]);
        session(['sid'=>$this->user['sid']]);
